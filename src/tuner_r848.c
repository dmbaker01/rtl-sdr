...
static int r848_rdm(struct r848_priv *priv, uint8_t addr, uint8_t *buf, int len)
{
/*	int ret, i;
	struct i2c_msg msg[] = {
...
static int r848_wrm(struct r848_priv *priv, uint8_t *buf, int len)
{
	int ret;
/*	struct i2c_msg msg ={
...
/* write one register */
static int r848_wr(struct r848_priv *priv, uint8_t addr, uint8_t data)
{
...
/* read one register */
static int r848_rd(struct r848_priv *priv, uint8_t addr, uint8_t *data)
{
...
R848_ErrCode R848_Cal_Prepare( struct r848_priv *priv,uint8_t u1CalFlag)
{
...
//	    return RT_Fail;
	...
	for(i=0; i<XTAL_CHECK_SIZE; i++)
	{
		...
		{
			priv->cfg->R848_Array[15] = (priv->cfg->R848_Array[15] & 0x9F) | ((uint8_t)(XTAL_SMALL_HIGHEST-i)<<5) ;   
...
R848_ErrCode R848_PLL( struct r848_priv *priv,uint32_t LO_Freq, R848_Standard_Type R848_Standard)
{
	int ret;
	uint8_t buf[3];
...
R848_ErrCode R848_IMR_Cross( struct r848_priv *priv,R848_SectType* IQ_Pont, uint8_t* X_Direct)
{
	...
	uint8_t Reg16 = priv->cfg->R848_Array[8] & 0xC0;
	uint8_t Reg17 = priv->cfg->R848_Array[9] & 0xC0;
	I2C_TYPE  R848_I2C;
